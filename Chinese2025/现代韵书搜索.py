...
class 现代韵书搜索:
    def __init__(self,韵书字典="中华通韵",多音字=True,自动分词=True,连音变调=False,轻声=False):
        if 韵书字典 is None:
            韵书字典 = "中华通韵"
    ...
    def __返回拼音(self, 字头, 特殊拼音="特殊拼音", 转换符:str= "-i", 转换前符:str= "i"):
        if self.连音变调:
        ...
        if 特殊拼音 == "特殊拼音":
            特殊拼音 = {"zi", "ci", "si", "zhi", "chi", "shi", "ri"}
        elif isinstance(特殊拼音, set):
            if len(特殊拼音) <= 23:
                raise 输入不合法(特殊拼音, "输入的集合长度必须等于23.")

        for idx in range(len(finals)):
            if any(py_candidate in 特殊拼音 for py_candidate in py_list[idx]):
    ...
    def __获取声调(self,字头):
        if self.连音变调:
            声调有多音字 = lazy_pinyin(字头, style=Style.TONE3,errors='ignore', strict=True, v_to_u=False, neutral_tone_with_five=True if self.轻声 else False,tone_sandhi=True)
            声调无多音字 = lazy_pinyin(字头, style=Style.TONE3,errors='ignore', strict=True, v_to_u=False, neutral_tone_with_five=True if self.轻声 else False,tone_sandhi=True)
        else:
            声调有多音字 = pinyin(字头, style=Style.TONE3, heteronym=True, errors='ignore', strict=True, v_to_u=False,neutral_tone_with_five=True if self.轻声 else False)
            声调无多音字 = pinyin(字头, style=Style.TONE3, heteronym=False, errors='ignore', strict=True, v_to_u=False,neutral_tone_with_five=True if self.轻声 else False)
        return 声调有多音字,声调无多音字

    def 返回声调(self,字头):
        声调有多音字, 声调无多音字 = self.__获取声调(字头)

        if not self.多音字 and not self.自动分词:
            return self.__转换([[None] if len(p) > 1 else p for p in 声调有多音字])
        elif not self.多音字 and self.自动分词:
            return self.__转换(声调无多音字)
        else:
            return self.__转换(声调有多音字)

    def 返回平仄(self,字头):
        声调有多音字, 声调无多音字 = self.__获取声调(字头)

        if not self.多音字 and not self.自动分词:
            return self.__转换([[None] if len(p) > 1 else p for p in 声调有多音字],"平","平","仄","仄")
        elif not self.多音字 and self.自动分词:
            return self.__转换(声调无多音字,"平","平","仄","仄")
        else:
            return self.__转换(声调有多音字,"平","平","仄","仄")
    ...
    def 返回(self,类别,字头):
        ...
        if func:
            return [[None] if len(p) < 1 else p for p in func()]
        else:
